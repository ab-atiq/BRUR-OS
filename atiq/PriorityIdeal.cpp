...
void calculateWaitingTime(int n, int bt[], int pri[], int wt[], int at[])
{
    ...
    int min_priority = 9999; // Minimum priority among the arrived processes
    ...
    while (complete != n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (bt[i] > 0 && at[i] <= time && pri[i] < min_priority)
            {
                highest = i;
                min_priority = pri[i];
            }
        ...
        time += bt[highest];
        bt[highest] = 0; // Set burst time to 0 to mark the process as completed
        complete++;
    }
}

...
void round_robin(int pid[], int arrival_time[], int burst_time[], int n, int time_quantum)
{
        ...
        for (int i = 0; i < n; i++)
        {
            if (arrival_time[i] <= current_time && remaining_time[i] > 0)
            {
                if (remaining_time[i] <= time_quantum)
                {
                    ...
                    remaining_time[i] = 0;
                    int waiting_time = current_time - burst_time[i] - arrival_time[i];
...
int main()
{
    ...
    int time_quantum = 2; // time quantum

    round_robin(pid, arrival_time, burst_time, n, time_quantum);
